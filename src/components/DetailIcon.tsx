import type { IconKey } from "@/content/products";
...
function Glyph({ icon }: { icon: IconKey }) {
  if (icon === "base") {
    // Base: its blue square on white.
    return <span className={styles.base} />;
  }
  if (icon === "somnia") {
    return (
      <svg className={styles.somnia} viewBox="0 0 42.1956 32" fill="currentColor" aria-hidden>
        {SOMNIA.map((d) => (
  ...
  const url = `url(/icons/${ICONS[icon].file}.svg)`;
  return <span className={styles.mask} style={{ maskImage: url, WebkitMaskImage: url, background: ICONS[icon].fg }} />;
}

export function DetailIcons({ icons }: { icons: IconKey[] }) {
  return (
    <ul className={styles.row}>
      {icons.map((icon, i) => (
        <li key={icon} className={styles.item} style={{ zIndex: icons.length - i }}>
          <span className={styles.label}>{ICONS[icon].name}</span>
          <span className={styles.icon} role="img" aria-label={ICONS[icon].name} style={{ background: ICONS[icon].bg, color: ICONS[icon].fg }}>
            <Glyph icon={icon} />
          </span>
        </li>
      ))}
